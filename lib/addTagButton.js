...
Template.newTagDialog.events({
	'submit form': function (event, template) {
		event.preventDefault();
		var node = template.find('input');
		//TODO: allow different collections
		Tags._getCollection().addTag({_id:this._id}, node.value);
	}
});
...
Template.newTagDialog.rendered = function () {
	...
	var listOfTags = Tags.tags.find({}, {reactive:false}).map(function (tag) {return tag.name});
